...
if (empty(!$_POST['login']) AND empty(!$_POST['mdp'])) // Pour verifier que les cases ne soit pas vide
	...
	$req = "select * from utilisateur where mail= '$login' and mdp = '$passe'";
	$res = mysql_query($req);
...
if(empty(!$_POST['mail']) AND empty(!$_POST['mdp']) AND empty(!$_POST['nom'])AND empty(!$_POST['prenom'])) // Gestion de validation des cases remplis
{
$login2 = $_POST["mail"];
@mysql_connect("localhost","root",""); // connexion SQL
mysql_select_db("ppe2");
$req5 = "SELECT * FROM `utilisateur` WHERE `mail` = '$login2' ";
...
mysql_select_db("ppe2");
$req2 = "insert into ppe2.utilisateur (nom, prenom, mail, mdp , IdGroupe) 
		values ('$nom', '$prenom', '$login', '$passe' , 2)"; // attention ! stageaire doit être parametrer par l'admin aprés coup ! 
	//echo $req2;
