...
// formateur (nom ou prenom)
	...
	$req = "SELECT * FROM realiser WHERE `ContenueFormateurNom` LIKE '%$formateurech%' or LIKE `ContenuFormateurPrenom` LIKE '%$formateurech%' " ;
}
// description
else if(!empty($_POST["motclerech"]) ){ 
	$req = "SELECT * FROM realiser WHERE  ContenueSequenceDescription LIKE '%$motclerech%' 
	OR  `ContenueFormateurNom` LIKE  '%$motclerech%' 
	OR  `ContenueFormateurPrenom` LIKE '%$motclerech%' 
